...
class _CalendarPageState extends State<CalendarPage> {
  ...
  // List of public holidays for the selected year.
  ...
  // Normalizes a DateTime to year, month, day.
  DateTime _normalizeDate(DateTime date) {
    return DateTime(date.year, date.month, date.day);
  }

  // Fetch Latvian public holidays from the API.
  Future<List<DateTime>> fetchLatvianHolidays(int year) async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var holiday in holidaysJson) {
        ...
        // Check for Mothers Day: if localName is "Mātes diena", do not add observed holiday.
        String? localName = holiday["localName"];
        bool isMatesDiena =
            localName != null &&
            localName.toString().toLowerCase() == "mātes diena";
        if (!isMatesDiena) {
          // If the holiday falls on Saturday or Sunday, add an observed holiday on Monday.
  ...
  // Builds a mapping from normalized date to total hours worked.
  Map<DateTime, double> _buildHoursMap(QuerySnapshot snapshot) {
    ...
    for (var doc in snapshot.docs) {
      ...
      if (data['date'] != null) {
        ...
        double dayHours = 0.0;
        // If the document contains an "entries" field, sum hours from each entry's tasks.
  ...
  // Builds a mapping from normalized date to its special status (if any).
  ...
  // Selected cell builder. (Removed unused 'status' variable.)
  ...
  // Custom header widget with month/year navigation and dropdowns.
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          return SingleChildScrollView(
            child: Column(
              children: [
                // Calendar Card with integrated custom header.
                Padding(
                ...
                // Details Card for Selected Date and Work Hours.
