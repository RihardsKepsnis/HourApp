...
void main() async {
  ...
  // Initialize locale data for Latvian.
...
/// HomePageContent displays a profile card, a weekly chart, and a monthly summary card.
...
class _HomePageContentState extends State<HomePageContent> {
  ...
  // New state variables for sick and holiday days.
  ...
  // Track the selected month for monthly summary.
  late DateTime _selectedMonth;
  // Track the starting Monday for the currently displayed week.
  late DateTime _selectedWeekStart;

  // Subscription for real-time monthly updates.
  ...
  void initState() {
    ...
    if (_selectedMonth.year == now.year && _selectedMonth.month == now.month) {
      // For the current month show the current week (starting from Monday).
      _selectedWeekStart = now.subtract(Duration(days: now.weekday - 1));
    } else {
      // For a selected month different from the current month, show the last week of that month.
  ...
  /// Fetch weekly work logs for the current week.
  Future<void> _fetchWeeklyWorkLogs() async {
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        double sumHours = 0.0;
        // Only change: if document contains an "entries" field, use it to sum hours,
        // otherwise fallback to the "tasks" field.
      ...
      // Recalculate the maximum Y value considering forced sick/holiday days.
      ...
      // Add a 10% margin and force a minimum max of 8.
  ...
  /// Navigate the weekly chart by deltaWeeks (can be -1 for previous week, +1 for next week).
  void _navigateWeek(int deltaWeeks) {
    DateTime potentialWeek = _selectedWeekStart.add(
      Duration(days: 7 * deltaWeeks),
    );
    // Only allow navigation if the new week is still within the selected month.
  ...
  /// Subscribe to monthly work logs in real time and compute the monthly summary.
  /// For each calendar day in the month, we determine if it is a working day.
  /// Normally, weekdays (Mon–Fri) are working days (with May 10 as a shifted working day).
  /// If a working day is not off (by public holiday or worker record) it defaults to 8 hours.
  /// Additionally, if the very next calendar day (regardless of month) is a public holiday (and not marked off),
  /// then the current day’s expected hours are reduced to 7.
  /// Also, we count the number of days marked as 'slimība' and 'atvaļinājums'.
  void _subscribeMonthlySummary() async {
    ...
    _monthlySubscription = FirebaseFirestore.instance
        ...
        .listen((snapshot) async {
          ...
          // Fetch all public holidays for the current year.
          ...
          for (int d = 1; d <= daysInMonth; d++) {
            ...
            // Determine if day is normally a working day (Mon–Fri).
            bool normallyWorking =
                day.weekday >= DateTime.monday &&
                day.weekday <= DateTime.friday;
            // For May 2025, treat May 10 as a shifted working day.
            ...
            if (isWorkingDay) {
              ...
              if (isPublicHoliday || workerOff) {
                ...
                dayExpected = 8.0;
                // Check the very next calendar day regardless of month.
          ...
          // Count sick days and worker holiday days.
          int sickDaysCount = 0;
  ...
  /// Fetch Latvian public holidays from the Nager.Date API.
  Future<List<DateTime>> fetchLatvianHolidays(int year) async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var holiday in holidaysJson) {
        ...
        // Optionally, skip International Women's Day.
        if (holidayDate.month == 3 && holidayDate.day == 8) continue;
        // If the holiday falls on Saturday or Sunday, add an observed holiday on the following Monday.
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Graph Card (weekly chart) with week navigation.
          Card(
            elevation: 12,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(20),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Week navigation with start and end date display.
                  Row(
          ...
          SizedBox(height: screenWidth * 0.03),
          // Monthly Summary Card with month navigation and sick/holiday counts.
          Card(
            elevation: 8,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(20),
